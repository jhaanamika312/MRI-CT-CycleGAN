...
dA_loss1_values = []  # Discriminator A loss on real images
dA_loss2_values = []  # Discriminator A loss on fake images
dB_loss1_values = []  # Discriminator B loss on real images
dB_loss2_values = []  # Discriminator B loss on fake images
g_loss1_values = []   # Generator AtoB loss
g_loss2_values = []   # Generator BtoA loss
...
def train(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, dataset, epochs=1):
    ...
    for i in range(n_steps):
        ...
        dA_loss1_values.append(dA_loss1)
        dA_loss2_values.append(dA_loss2)
        dB_loss1_values.append(dB_loss1)
        dB_loss2_values.append(dB_loss2)
        g_loss1_values.append(g_loss1)
        g_loss2_values.append(g_loss2)

        import pickle

        # Specify the directory path where you want to save the loss values file
        directory_path = '/home/user/jha2a/cyclegan/'

    # Save the loss values after each iteration
        loss_values = {
            'dA_loss1_values': dA_loss1_values,
            'dA_loss2_values': dA_loss2_values,
            'dB_loss1_values': dB_loss1_values,
            'dB_loss2_values': dB_loss2_values,
            'g_loss1_values': g_loss1_values,
            'g_loss2_values': g_loss2_values
        }
    ...
    with open(file_path, 'wb') as file:
        ...
        # Since our batch size = 1, the number of iterations would be the same as the size of our dataset.
        ...
        if (i+1) % (bat_per_epo * 5) == 0:
            # save the models
            # If batch size (total images) = 100, the model will be saved after
